...
{
    public class AccessorInstanceCallPrivateMethodGenericUnitTest : UnitTest
    {
        public override string CommandActivator => "AccessorInstanceCallPrivateMethodGeneric";

        public override bool IsExpectedResult(object[] result)
        {
            if (!(result[0] is int))
            {
                debug.Log(CommandActivator + ": Expected: Int32, Got: " + result[0].GetType());
                return false;
            }
            if ((int)result[0] != 5)
            {
            ...
            if (!(result[1] is int))
            {
                debug.Log(CommandActivator + ": Expected: Int32, Got: " + result[1].GetType());
                return false;
            }
